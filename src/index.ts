...
async function main() {
  try {
    ...
    const parentOasHash =
      getInput("parent_oas_hash", { required: false }) || undefined;
    const parentConfigHash =
      getInput("parent_config_hash", { required: false }) || undefined;
    const parentBranch =
      getInput("parent_branch", { required: false }) || undefined;
    ...
    // create a new build
    const build = await stainless.builds
      .create({
        projectName,
        ...
        stainlessConfig:
          configPath && configBuffer
            ? new File([configBuffer], path.basename(configPath), {
                type: "text/plain",
    ...
    const languageHeader = build.headers.get(
      "X-Stainless-Project-Build-Languages",
    );
    let languages = (
      languageHeader?.length ? languageHeader.split(",") : []
    ) as Stainless.Builds.OutputRetrieveParams["target"][];
    if (buildId) {
      console.log(
      ...
      if (!buildId) {
        console.log(
          `No new build was created. Checking for existing builds with the inputs provided...`,
        );
        const build = (
          await stainless.builds.list({
            project: projectName,
            spec_hash: crypto.createHash("md5").update(oasBuffer).digest("hex"),
            config_hash: configBuffer
              ? crypto.createHash("md5").update(configBuffer).digest("hex")
              : undefined,
            branch,
            limit: 1,
          })
        ...
        if (build) {
          ...
          console.log(
            `Found existing build with ID ${buildId} for languages: ${languages.join(", ")}`,
          );
        }
      }
      ...
      if (!buildId) {
        console.error(
          "No existing build was found for this branch. Presumably it does not include SDK config changes",
        );
        process.exit(0);
      }
    }
    ...
    let parentOutcomes: Record<
      string,
      Stainless.Builds.Outputs.CommitBuildStep.Completed["completed"]
    > = {};
    let outcomes: Record<
      string,
      Stainless.Builds.Outputs.CommitBuildStep.Completed["completed"]
    ...
    while (
      ...
      for (const language of languages) {
        if (!(language in parentOutcomes) && parentBuildId) {
          const parentBuildOutput = await stainless.builds.outputs.retrieve(
            parentBuildId,
            { target: language },
          );
        ...
        if (!(language in outcomes)) {
          const buildOutput = await stainless.builds.outputs.retrieve(buildId, {
            target: language,
          });
